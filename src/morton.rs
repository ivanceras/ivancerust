...
impl Morton{
	...
	pub fn point_at_lod(&self, x:u64, y:u64, z:u64, new_lod:u8)->(i64, i64, i64){
		...
		(xnew as i64, ynew as i64, znew as i64)
	}
	
	pub fn linear_index(&self, morton:u64)->u64{
		let (x, y, z) = self.decode(morton);
		x * self.limit * self.limit + y * self.limit + z
	}
	
	//from linear index to x,y,z
	pub fn from_linear_index(&self, idx:u64)->(u64, u64, u64){
	    let mut index = idx;
